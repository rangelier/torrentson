...
{
    /// <summary>
    /// The parser for eztv feeds.
    /// </summary>
    public class EzTvRssParser : IRssParser
    {
        /// <summary>
        ...
        public IEnumerable<Torrent> Parse(string xml, params string[] keywords)
        {
            ...
            return from item in document.Descendants("item")
                   ...
                   select new Torrent
                   {
                       ...
                       : string.Empty,
                       Magnet = item.Element(ns + "torrent") != null && item.Element(ns + "torrent").Element(ns + "magnetURI") != null
                       ? item.Element(ns + "torrent").Element(ns + "magnetURI").Value
                       : string.Empty,
